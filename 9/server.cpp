...
/* 客户数据：客户端socket地址、代写到客户端的数据的位置、从客户端读取的数据 */
struct client_data{
    sockaddr_in address;
    char* write_buf;
...
int main(int argc, const char* argv[])
{
     ...
     * 每个可能的socket连接都可以获得这样的对象，并且socket的值可以直接
     * 用来索引（作为数组的下标）socket连接对应的client_data对象，
     * 这是将socket和客户数据关联的简单而高效的方式
     */
    client_data* users = new client_data[FD_LIMIT];
    /*
    * 尽管我们分配了足够多的client_data对象，但是为了提高poll的性能，
    * 仍然有必要限制用户的数量
    */
