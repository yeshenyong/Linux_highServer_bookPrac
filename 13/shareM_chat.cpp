...
static const char* shm_name = "/my_shm";    /* static静态放在全局区的静态区 */
int sig_pipefd[2];
int epollfd;
int listenfd;
int shmfd;
...
void sig_handler( int sig )
{
    ...
    int msg = sig;
    send( sig_pipefd[1], ( char* )&msg, 1, 0 ); /* 往管道写入数据，通知sig_pipe[0] */
    errno = save_error;
}
...
int run_child( int idx, client_data* users, char* share_mem )
{
    ...
    /* 子进程使用I/O复用技术来同时监听两个文件描述符：客户连接socket、与父进程通信的管道文件文件描述符 */
    ...
    while( !stop_child )
    {
        ...
        for( int i = 0; i < number; i++ )
        {
            ...
            if( ( sockfd == connfd ) && ( events[i].events & EPOLLIN ) )
            {
                ...
                /*
                    将客户数据读取到对应的读缓冲中。该读缓存是共享内存的一段，它开始于 idx * BUFFER_SIZE
    ...
    while( !stop_server )
    {
        ...
        for( int i = 0; i < number; ++i )
        {
            ...
            else if( events[i].events & EPOLLIN )
            {
                ...
                {
                    /* 向除负责处理第child个客户连接的子进程之外的其他进程发送消息，通知它们有客户数据要写 */
